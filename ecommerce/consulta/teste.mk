# Explicação detalhada das anotações usadas no código fornecido:

# @ControllerAdvice
# A anotação @ControllerAdvice é usada para definir uma classe que contém código de tratamento global de exceções para os controladores da aplicação.
# Isso significa que qualquer exceção lançada por um controlador pode ser tratada por métodos definidos nesta classe.
# É uma forma de centralizar o tratamento de exceções em um único lugar.

# @ExceptionHandler
# A anotação @ExceptionHandler é usada para definir um método que será invocado automaticamente quando uma exceção específica for lançada em qualquer controlador.
# No código fornecido, há dois métodos anotados com @ExceptionHandler, cada um tratando um tipo específico de exceção:
...
# @Autowired
# Embora não esteja explicitamente no trecho fornecido, a anotação @Autowired é frequentemente usada em classes de serviço e controladores para injetar dependências automaticamente.
# No contexto de um ControllerAdvice, ela pode ser usada para injetar serviços que ajudam no tratamento de exceções.

# @Transactional
# Novamente, não está no trecho fornecido, mas a anotação @Transactional é usada para gerenciar transações em métodos de serviço.
# Ela garante que todas as operações dentro do método sejam executadas dentro de uma transação, e se qualquer operação falhar, todas as mudanças serão revertidas.
